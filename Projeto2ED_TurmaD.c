/*Alunos : Arthur Augusto Pinto Cunha -- matricula : 16/0113008;
         : Arthur Alves Rodrigues Pinto -- Matricula: 16/0112974;

Professor: Dibio - Estrutura de Dados
...
int contadorDe1 = 0; // verificar quantas aeronaveis tem combustivel = 1
...
//Libera pousa na pista 2
void Pista2(voos* FILA)
{
	voos* aux = FILA->prox;
	voos* aux1 = aux->prox;
	...
	//SÓ libera a aterrissagem se o combustivel for 1
	   ...
	   if(aux->numDoVoo <= 9)
		  printf ("-> Aeronave 0%d LIBERADA A ATERRISSAGEM NA PISTA 2\n", aux1->numDoVoo);
	   else
          printf ("-> Aeronave %d LIBERADA A ATERRISSAGEM NA PISTA 2\n", aux1->numDoVoo);

	}
}
//Libera pousa na pista 3
void Pista3(voos* FILA)
{
	voos* aux = FILA->prox;
	voos* aux1 = aux->prox;
	voos* aux2 = aux1->prox;
	...
	//SÓ libera a aterrissagem se o combustivel for 1
	   ...
	   if(aux->numDoVoo <= 9)
		  printf ("-> Aeronave 0%d LIBERADA A ATERRISSAGEM NA PISTA DE EMERGENCIA\n", aux2->numDoVoo);
	   else
          printf ("-> Aeronave %d LIBERADA A ATERRISSAGEM NA PISTA DE EMERGENCIA\n", aux2->numDoVoo);
...
//Cria fila de avioes no deposito
	...
	//QUANDO APROXIMACOES É ZERO DECOLAR TODOS OS VOOS
	if(NDecolagens > 0 && NAproximacoes == 0){

	   for(int i =0; i<NDecolagens ; i++){
        Decolar(POUSADOS,NDecolagens, tempo);
	   }
	}
